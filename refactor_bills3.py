...
# Find the old function start - doc comment end then pub fn
old_func_start = content.find('/// year/month 传 None 表示不筛选；传 Some 则拼接 WHERE year_month = ?' + L + L + 'pub fn query_bills_tx(')
if old_func_start < 0:
    # Try alternative
    old_func_start = content.find('pub fn query_bills_tx(')
    while old_func_start > 0 and not content[old_func_start-50:old_func_start].strip().startswith('///'):
        old_func_start = content.find('pub fn query_bills_tx', old_func_start+1)
    print(f'Found pub fn at: {old_func_start}')
    # Go back to the doc comment
    while old_func_start > 0 and not content[old_func_start-3:old_func_start] == '/// ' + L + '///':
        old_func_start -= 1
        if old_func_start == 0:
            break
    print(f'Re-adjusted to: {old_func_start}')

old_func_end = content.find('/// 账单行（操作前校验用）', old_func_start)
print(f'Old function start: {old_func_start}, end: {old_func_end}')
